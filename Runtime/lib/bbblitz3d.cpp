...
static inline void debugParent(Entity* e)
{
	if (debug) {
		...
		if (e && !entity_set.count(e))
			ThrowRuntimeException("Parent entity does not exist");
	}
}
static inline void debugMesh(MeshModel* m)
{
	if (debug) {
		debugEntity(m);
		if (!m->getMeshModel())
			ThrowRuntimeException("Entity is not a mesh");
	}
}
static inline void debugObject(Object* o)
{
	if (debug) {
		debugEntity(o);
		if (!o->getObject())
			ThrowRuntimeException("Entity is not an object");
	}
}
static inline void debugColl(Object* o, int index)
{
	if (debug) {
		...
		if (index < 1 || index > o->getCollisions().size())
			ThrowRuntimeException("Collision index out of range");
	}
}
static inline void debugCamera(Camera* c)
{
	if (debug) {
		debugEntity(c);
		if (!c->getCamera())
			ThrowRuntimeException("Entity is not a camera");
	}
}
static inline void debugLight(Light* l)
{
	if (debug) {
		debugEntity(l);
		if (!l->getLight())
			ThrowRuntimeException("Entity is not a light");
	}
}
static inline void debugModel(Model* m)
{
	if (debug) {
		debugEntity(m);
		if (!m->getModel())
			ThrowRuntimeException("Entity is not a model");
	}
}
static inline void debugSprite(Sprite* s)
{
	if (debug) {
		debugModel(s);
		if (!s->getSprite())
			ThrowRuntimeException("Entity is not a sprite");
	}
}
static inline void debugMD2(MD2Model* m)
{
	if (debug) {
		debugModel(m);
		if (!m->getMD2Model())
			ThrowRuntimeException("Entity is not an MD2 Model");
	}
}
static inline void debugBSP(Q3BSPModel* m)
{
	if (debug) {
		debugModel(m);
		if (!m->getBSPModel())
			ThrowRuntimeException("Entity is not a BSP Model");
	}
}
static inline void debugTerrain(Terrain* t)
{
	if (debug) {
		debugModel(t);
		if (!t->getTerrain())
			ThrowRuntimeException("Entity is not a terrain");
	}
}
static inline void debugSegs(int n)
{
	if (debug) {
		debug3d();
		if (n < 3 || n > 50)
			ThrowRuntimeException("Illegal number of segments");
	}
}
static inline void debugVertex(Surface* s, int n)
{
	if (debug) {
		...
		if (n < 0 || n >= s->numVertices())
			ThrowRuntimeException("Vertex index out of range");
	}
}
static inline void debugVertex(Surface* s, int n, int t)
{
	if (debug) {
		...
		if (n < 0 || n >= s->numVertices())
			ThrowRuntimeException("Vertex index out of range");
		if (t < 0 || t > 1)
			ThrowRuntimeException("Texture coordinate set out of range");
	}
}
#endif
...
///////////////////////////
void bbLoaderMatrix(BBStr* ext, float xx, float xy, float xz, float yx, float yy, float yz, float zx, float zy,
					float zz)
{
...
void bbRenderWorld(float tween)
{
	...
	string t_fps  = "000" + itoa(fps);
	t_fps         = t_fps.substr(t_fps.size() - 4);
	string t_ups  = "000" + itoa(ups);
	t_ups         = t_ups.substr(t_ups.size() - 4);
	string t_rps  = "000" + itoa(rps);
	t_rps         = t_rps.substr(t_rps.size() - 4);
	string t_tris = "00000" + itoa(tris);
	t_tris        = t_tris.substr(t_tris.size() - 6);
...
void bbAddMesh(MeshModel* src, MeshModel* dest)
{
	...
	if (debug) {
		...
		debugMesh(dest);
		if (src == dest)
			ThrowRuntimeException("A mesh cannot be added to itself");
	}
#endif
	dest->add(*src);
}
...
int bbCameraProject(Camera* c, float x, float y, float z)
{
	...
	if (v.z > 0) {
		...
		if (v.z <= fr) {
			...
			float nr_w = c->getFrustumWidth();
			float nr_h = c->getFrustumHeight();
			projected  = Vector((v.x * nr / v.z / nr_w + .5f) * vp_w, (.5f - v.y * nr / v.z / nr_h) * vp_h, nr);
			return 1;
		}
	}
	projected = Vector();
...
int bbEntityInView(Entity* e, Camera* c)
{
	...
	if (Model* p = e->getModel()) {
		if (MeshModel* m = p->getMeshModel()) {
			...
			Vector     p[] = {t * b.corner(0), t * b.corner(1), t * b.corner(2), t * b.corner(3),
...
Entity* bbLoadSprite(BBStr* file, int flags, Entity* p)
{
	...
	if (flags & gxCanvas::CANVAS_TEX_MASK)
		s->setBlend(gxScene::BLEND_REPLACE);
	else if (flags & gxCanvas::CANVAS_TEX_ALPHA)
		s->setBlend(gxScene::BLEND_ALPHA);
	else
		s->setBlend(gxScene::BLEND_ADD);
...
Entity* bbCreateTerrain(int n, Entity* p)
{
	...
	while ((1 << shift) < n)
		++shift;
	if ((1 << shift) != n)
		ThrowRuntimeException("Illegal terrain size");
...
Entity* bbLoadTerrain(BBStr* file, Entity* p)
{
	...
	if (!c)
		ThrowRuntimeException("Unable to load heightmap image");
	int w = c->getWidth(), h = c->getHeight();
	if (w != h)
		ThrowRuntimeException("Terrain must be square");
	int shift = 0;
	while ((1 << shift) < w)
		++shift;
	if ((1 << shift) != w)
...
Entity* bbCreateListener(Entity* p, float roll, float dopp, float dist)
{
	...
	if (debug) {
		debugParent(p);
		if (listener)
			ThrowRuntimeException("Listener already created");
	}
#endif
...
gxChannel* bbEmitSound(gxSound* sound, Object* o)
{
	...
	if (debug) {
		debugObject(o);
		if (!listener)
			ThrowRuntimeException("No Listener created");
	}
#endif
...
void bbSetAnimKey(Object* o, int frame, int pos_key, int rot_key, int scl_key)
{
	...
	if (pos_key)
		anim.setPositionKey(frame, o->GetLocalPosition());
	if (rot_key)
		anim.setRotationKey(frame, o->GetLocalRotation());
	if (scl_key)
		anim.setScaleKey(frame, o->GetLocalScale());
	o->setAnimation(anim);
}
...
void bbTFormPoint(float x, float y, float z, Entity* src, Entity* dest)
{
	...
	if (src)
		tformed = src->GetWorldTransform() * tformed;
	if (dest)
		tformed = -dest->GetWorldTransform() * tformed;
}
...
void bbTFormVector(float x, float y, float z, Entity* src, Entity* dest)
{
	...
	tformed = Vector(x, y, z);
	if (src)
		tformed = src->GetWorldTransform().m * tformed;
	if (dest)
		tformed = -dest->GetWorldTransform().m * tformed;
}
...
void bbTFormNormal(float x, float y, float z, Entity* src, Entity* dest)
{
	...
	tformed = Vector(x, y, z);
	if (src)
		tformed = (src->GetWorldTransform().m).cofactor() * tformed;
	if (dest)
		tformed = (-dest->GetWorldTransform().m).cofactor() * tformed;
	tformed.normalize();
}
...
void bbEntityType(Object* o, int type, int recurs)
{
	...
	if (debug) {
		debugObject(o);
		if (type < 0 || type > 999)
			ThrowRuntimeException("EntityType ID must be in the range 0...999");
	}
#endif
	if (recurs)
		entityType(o, type);
...
void bbTurnEntity(Entity* e, float p, float y, float r, int global)
{
	...
	global
		? e->SetWorldRotation(rotationQuat(p * s_degreesToRadians, y * s_degreesToRadians, r * s_degreesToRadians)
							  * e->GetWorldRotation())
		: e->SetLocalRotation(e->GetLocalRotation()
							  * rotationQuat(p * s_degreesToRadians, y * s_degreesToRadians, r * s_degreesToRadians));
}
...
void bbTranslateEntity(Entity* e, float x, float y, float z, int global)
{
	...
	debugEntity(e);
#endif
	global ? e->SetWorldPosition(e->GetWorldPosition() + Vector(x, y, z))
...
void bbPositionEntity(Entity* e, float x, float y, float z, int global)
{
	...
	debugEntity(e);
#endif
	global ? e->SetWorldPosition(Vector(x, y, z)) : e->SetLocalPosition(Vector(x, y, z));
}
...
void bbScaleEntity(Entity* e, float x, float y, float z, int global)
{
	...
	debugEntity(e);
#endif
	global ? e->SetWorldScale(Vector(x, y, z)) : e->SetLocalScale(Vector(x, y, z));
}
...
void bbRotateEntity(Entity* e, float p, float y, float r, int global)
{
	...
	debugEntity(e);
#endif
	global ? e->SetWorldRotation(rotationQuat(p * s_degreesToRadians, y * s_degreesToRadians, r * s_degreesToRadians))
...
BBStr* bbEntityClass(Entity* e)
{
	...
	if (e->getLight())
		...
		if (t->getSprite())
			...
			p = "Terrain";
		else if (t->getPlaneModel())
			p = "Plane";
		else if (t->getMeshModel())
			p = "Mesh";
		else if (t->getMD2Model())
			p = "MD2";
		else if (t->getBSPModel())
			p = "BSP";
	}
	return new BBStr(p);
}
...
{
	if (e) {
		while (Entity::GetEntityOrphans())
			bbFreeEntity(Entity::GetEntityOrphans());
	}
	if (b) {
		while (brush_set.size())
			bbFreeBrush(*brush_set.begin());
	}
	if (t) {
		while (texture_set.size())
			bbFreeTexture(*texture_set.begin());
	}
}
...
void blitz3d_open()
{
	...
	if (!gx_scene)
		ThrowRuntimeException("Unable to create 3D Scene");
	world            = new World();
	projected        = Vector();
	picked.collision = Collision();
	picked.with      = 0;
	picked.coords    = Vector();
	Texture::clearFilters();
