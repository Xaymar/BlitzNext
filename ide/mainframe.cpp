...
enum { TAB_INVALID, TAB_EDITOR, TAB_HTMLHELP, TAB_DEBUGLOG };
...
int MainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	static UINT    toolbuts[] = {Inew,         ID_OPEN,  ID_SAVE,      ID_CLOSE,  ID_SEPARATOR, ID_CUT,
                              ID_COPY,      ID_PASTE, ID_SEPARATOR, ID_FIND,   ID_SEPARATOR, ID_EXECUTE,
                              ID_SEPARATOR, ID_HOME,  ID_BACK,      ID_FORWARD};
...
bool MainFrame::open(const string& f)
{
	...
	if (!file.size() || (attr & FILE_ATTRIBUTE_DIRECTORY)) {
		...
		if (GetFullPathName(file.c_str(), MAX_PATH, buff, &p))
			file = buff;
		else
			file = f;
	}
...
{
	id -= 333;
	if (id < 0 || id >= prefs.recentFiles.size())
		return;
...
{
#ifdef DEMO
	MessageBox("Create Executable unavailable in demo version", "Sorry!",
			   MB_TOPMOST | MB_SETFOREGROUND | MB_ICONINFORMATION);
	return;
#endif
	Editor* e = getEditor();
	if (!e)
	...
	if (prefs.prg_debug) {
		...
		if (MessageBox(t.c_str(), 0, MB_OKCANCEL) == IDCANCEL)
			return;
	}
	build(false, true);
}
...
{
	static char* dirs[] = {"help\\commands\\2d_commands\\", "help\\commands\\3d_commands\\", 0};
